...
public class Vehicle_category_Dao extends connectionInitializer{
 ...
 int value=stmt.executeUpdate();
  ...
  public ArrayList getVehicleCat() throws Exception{
   ArrayList al= new ArrayList();
   com.pojo.Vehicle_Category vc;
   stmt=conn.prepareStatement("select * from vehicle_category");
   rs=stmt.executeQuery();
   while(rs.next()){
   vc= new com.pojo.Vehicle_Category();
   vc.setVehicle_cat_id(rs.getInt(1));
   vc.setVehicle_cat_name(rs.getString(2));
   al.add(vc);
   }
    return al;
    }
  public ArrayList getVehicleCat_name(int id) throws Exception{
   ArrayList al= new ArrayList();
   com.pojo.Vehicle_Category vc;
   stmt=conn.prepareStatement("select vehicle_cat_name from vehicle_category where vehicle_cat_id='"+id+"'");
   rs=stmt.executeQuery();
   while(rs.next()){
   vc= new com.pojo.Vehicle_Category();
   //vc.setVehicle_cat_id(rs.getInt(1));
   vc.setVehicle_cat_name(rs.getString(1));
   al.add(vc);
   }
    return al;
    }
